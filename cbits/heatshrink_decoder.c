...
heatshrink_decoder *heatshrink_decoder_alloc(uint16_t input_buffer_size,
    ...
    LOG("-- decoder : %p, buffers: %p\n", hsd, hsd->buffers);
    return hsd;
}

void heatshrink_decoder_free(heatshrink_decoder *hsd) {
    LOG("-- heatshrink_decoder_free %p\n", hsd);
    if (hsd != NULL) {
        size_t buffers_sz = (1 << hsd->window_sz2) + hsd->input_buffer_size;
        size_t sz = sizeof(heatshrink_decoder);
        HEATSHRINK_FREE(hsd->buffers, buffer_sz);
        HEATSHRINK_FREE(hsd, sz);
        (void)buffers_sz;   /* may not be used by free */
        (void)sz;
    }
}
#endif
...
void heatshrink_decoder_reset(heatshrink_decoder *hsd) {
    LOG("-- heatshrink_decoder_reset %p\n", hsd);
...
HSD_sink_res heatshrink_decoder_sink(heatshrink_decoder *hsd,
    ...
    LOG("-- heatshrink_decoder_sink %p %p %lu %p\n", hsd, in_buf, size, input_size);
...
HSD_poll_res heatshrink_decoder_poll(heatshrink_decoder *hsd,
    ...
    LOG("-- heatshrink_decoder_poll %p %p %lu %p\n", hsd, out_buf, out_buf_size, output_size);
